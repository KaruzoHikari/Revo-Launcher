...
namespace UnityEngine.UI.Extensions.ColorPicker
{
	public class ColorPickerPresets : MonoBehaviour
	{
		...
		public virtual void CreatePreset(Color color, bool loading)
		{
			...
			newPresetButton.GetComponent<LongClickButton>().onLongClick.AddListener(() =>
			{
				DeletePreset(newPresetButton, newPresetButton.GetComponent<Image>().color);
			});
		...
		public virtual void DeletePreset(GameObject prefab, Color color)
		{
			Destroy(prefab);
			presets.Remove(color);
			createButton.gameObject.SetActive(presets.Count < maxPresets);
			SavePresets(saveMode);
		}
